"""--- Day 1: Historian Hysteria ---
    ...
    a group of Senior Historians has asked you to accompany them as they check the places
    ...
    a unique number called the location ID. To make sure they don't miss anything, The Historians split into two groups,
...
class Day(DayBase):
    def do(self):
        self.test(11, 31, example, example)
        self.run(1_530_215, 26_800_609)
    ...
    def part1(data=example):
        ...
        counter = 0
        for lin in lines:
            no_alpha = re.split(r"\s+", lin)
            l1.append(int(no_alpha[0]))
            l2.append(int(no_alpha[1]))
        l1.sort()
        l2.sort()
    ...
    def part2(data=example):
        ...
        but in the commotion you notice an interesting detail: a lot of location IDs appear in both lists! c
        ...
        for lin in lines:
            ...
            l1.append(int(no_alpha[0]))
            l2.append(int(no_alpha[1]))
        u1 = set(l1).intersection(l2)
        values1, counts1 = numpy.unique(l1, return_counts=True)
        values2, counts2 = numpy.unique(l2, return_counts=True)
        for i in u1:
            counter += i * int(counts1[numpy.where(values1 == i)[0]]) * int(counts2[numpy.where(values2 == i)[0]])
        return counter
